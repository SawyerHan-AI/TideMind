...
describe('metabolism Worker packaged candidate', () => {
  it('is a built bin entry covered by asarUnpack with native dependencies', () => {
    ...
    const nativeExternals = buildScript.match(/const NATIVE_EXTERNALS = \[([^\]]+)\]/u)?.[1] ?? ''
    expect(nativeExternals).toContain("'better-sqlite3'")
    expect(nativeExternals).toContain("'sqlite-vec'")
