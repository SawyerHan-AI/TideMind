...
import { getNodesByIds } from '../../src/db/nodes.js';
import { makeNode } from '../helpers/fixtures.js';

const db = {} as any; // mock db，不会被直接使用
...
describe('searchVector', () => {
  it('embedding 获取失败返回空数组', async () => {
    ...
    const results = await searchVector(db, 'test query');
  ...
  it('无向量结果返回空数组', async () => {
    ...
    const results = await searchVector(db, 'test');
    expect(results).toEqual([]);
  });
  ...
  it('正常返回结果并转换 L2 距离为相似度', async () => {
    ...
    vi.mocked(getNodesByIds).mockReturnValue(new Map([['n1', node1]]));
  ...
  it('过滤掉极低 heat 的节点', async () => {
    vi.mocked(getEmbedding).mockResolvedValue(new Float32Array(10));
    vi.mocked(searchVectors).mockReturnValue([{ id: 'n1', distance: 0.5 }]);
    vi.mocked(getNodesByIds).mockReturnValue(new Map([
  ...
  it('按 type 过滤', async () => {
    ...
    const results = await searchVector(db, 'test', { type: 'fact' });
    expect(results).toHaveLength(1);
  ...
  it('excludeMeta 过滤 meta 节点', async () => {
    ...
    const results = await searchVector(db, 'test', { excludeMeta: true });
    expect(results).toHaveLength(0);
  });
  ...
  it('createdAfter 过滤', async () => {
    ...
    const results = await searchVector(db, 'test', { createdAfter: '2026-01-01' });
    expect(results).toHaveLength(0);
  });
  ...
  it('createdBefore 过滤', async () => {
    ...
    const results = await searchVector(db, 'test', { createdBefore: '2026-01-01' });
    expect(results).toHaveLength(0);
  });
  ...
  it('有 context 时拼接到 embedding 文本', async () => {
    ...
    await searchVector(db, 'query', { context: '背景信息' });
  ...
  it('limit 参数生效', async () => {
    ...
    const results = await searchVector(db, 'test', { limit: 2 });
