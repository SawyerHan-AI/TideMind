...
const t = (key) => i18n.t(key, { ns: 'constants' });
// --- 派生分类（从维度计算） ---
export const DERIVED_CATEGORY_COLORS = nodeColors;
export function getDerivedCategoryLabel(key) {
    return t(`derivedCategory.${key}`);
}
export const DERIVED_CATEGORY_BG = {
...
export function getNodeTypeLabel(key) {
    return t(`nodeType.${key}`);
}
/** @deprecated 改用 DERIVED_CATEGORY_BG */
export const NODE_TYPE_BG = {
...
/** 关系类型 → 颜色分组 */
export const RELATION_COLORS = relationColors;
export function getRelationLabel(key) {
    return t(`relation.${key}`);
}
export function getGateLabel(key) {
    return t(`gate.${key}`);
}
...
export function getGateInfo(key) {
...
export function getEventSubtypeLabel(key) {
    return t(`eventSubtype.${key}`);
}
// --- 触发角色 ---
export function getActorLabel(key) {
    return t(`actor.${key}`);
}
...
/** 关系类型 → 线型（图视图用） */
export const RELATION_LINE_STYLES = {
    caused_by: 'solid',
    continues: 'solid',
    updates: 'solid',
    supports: 'solid',
...
export const OPERATION_CATEGORY_MAP = {
    // 记忆
    digest: 'memory',
    'annotate': 'memory',
    dedup: 'memory',
...
export function getOperationLabel(key) {
...
export function getSourceToolLabel(key) {
...
function i18nRecord(prefix, keys) {
    const base = {};
    for (const k of keys)
        base[k] = ''; // placeholder
...
export const GATE_LABELS = new Proxy({}, {
    get(_target, prop) {
