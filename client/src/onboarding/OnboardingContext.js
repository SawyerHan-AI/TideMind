...
export const STEP_KEYS = ['welcome', 'preferences', 'model', 'agent', 'noteSource', 'cloudSync', 'complete'];
export function OnboardingProvider({ children, onFinish }) {
    ...
    useEffect(() => {
        ...
        const checkAll = async () => {
            ...
            try {
                ...
                if (cancelled)
                    return;
                const hasOnline = Array.isArray(connections) && connections.some((c) => c.status === 'online' && !c.archived);
                const hasAgents = Array.isArray(agents) && agents.length > 0;
                const hasNoteSources = Array.isArray(sources) && sources.length > 0;
                // 避免每 tick 都触发 re-render：只在值真变时 setState
                if (pollingRef.current.modelConfigured !== hasOnline)
                    setModelConfigured(hasOnline);
                if (pollingRef.current.agentConfigured !== hasAgents)
                    setAgentConfigured(hasAgents);
                if (pollingRef.current.noteSourceConfigured !== hasNoteSources)
                    setNoteSourceConfigured(hasNoteSources);
            }
            catch { /* ignore */ }
        };
        checkAll();
    ...
    return (_jsx(OnboardingCtx.Provider, { value: {
            currentStep,
            totalSteps,
            modelConfigured,
            agentConfigured,
