...
const DataChangeContext = createContext({ revision: 0, lastScopes: [] });
export function DataChangeProvider({ children }) {
    const [state, setState] = useState({ revision: 0, lastScopes: [] });
    useEffect(() => {
    ...
    return (_jsx(DataChangeContext.Provider, { value: state, children: children }));
}
/**
 ...
 */
export function useDataRevision(scopes) {
