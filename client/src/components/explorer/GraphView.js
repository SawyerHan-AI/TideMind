...
/* ---------- component ---------- */
export function GraphView({ filter, selectedId, onSelect, graphLimit = 500, onGraphLimitChange }) {
    ...
    const [structureHoles, setStructureHoles] = useState([]);
    // data fetching
    ...
    const { data: graphData } = useIPC(() => window.api.nodes.graph(filter), [
    ...
    const adjacency = useMemo(() => {
        ...
        for (const l of graphData.links) {
            if (!map.has(l.from_id))
                map.set(l.from_id, new Set());
            if (!map.has(l.to_id))
                map.set(l.to_id, new Set());
    ...
    const neighborhoodSet = useMemo(() => {
        if (!selectedId || neighborhoodDepth === 0)
            return null;
    ...
    const selectedNeighbors = useMemo(() => {
        if (!selectedId)
            return new Set();
        return adjacency.get(selectedId) ?? new Set();
    ...
    const render = useCallback(() => {
        ...
        const isVisible = (id) => {
            if (connectivityThreshold > 0) {
                ...
                if (n && n.connectivity < connectivityThreshold)
                    return false;
            }
            if (neighborhoodSet && !neighborhoodSet.has(id))
                return false;
            return true;
        };
        ...
        for (const link of links) {
            ...
            if (source.x == null || source.y == null || target.x == null || target.y == null)
                continue;
            if (!isVisible(source.id) || !isVisible(target.id))
                continue;
    ...
    useEffect(() => {
        ...
        if (existing) {
            ...
            const linkForce = existing.force('link');
            if (linkForce)
                linkForce.links(links);
            // 居中力随窗口尺寸更新(尺寸变化时也走这里)
            const centerForce = existing.force('center');
            if (centerForce)
        ...
        const sim = forceSimulation(nodes)
            ...
            .force('collision', forceCollide().radius(d => nodeRadius(d) + 2))
            .alphaDecay(0.02)
    ...
    useEffect(() => {
        ...
        const resizeCanvas = () => {
            ...
            if (simulationRef.current) {
                const center = simulationRef.current.force('center');
                if (center) {
    ...
    useEffect(() => {
        ...
        const handlePointerUp = (e) => {
            ...
            if (dx * dx + dy * dy > 25 || Date.now() - down.time > 500)
                return;
                ...
                if (pathMode) {
                    if (!pathFrom) {
                        ...
                        window.api.nodes.path(pathFrom, node.id)
                            .then(result => { if (mountedRef.current)
    ...
    return (_jsxs("div", { ref: containerRef, className: "relative w-full h-full", children: [_jsx("canvas", { ref: canvasRef, className: "absolute inset-0 w-full h-full", style: { cursor: pathMode ? 'crosshair' : 'default' } }), _jsx(GraphToolbar, { neighborhoodDepth: neighborhoodDepth, onNeighborhoodDepthChange: setNeighborhoodDepth, pathMode: pathMode, pathFrom: pathFrom, onPathModeToggle: handlePathModeToggle, showStructureHoles: showStructureHoles, onStructureHolesToggle: () => setShowStructureHoles(v => !v), onZoomIn: handleZoomIn, onZoomOut: handleZoomOut, onFitAll: handleFitAll, connectivityThreshold: connectivityThreshold, onConnectivityThresholdChange: setConnectivityThreshold, maxConnectivity: maxConnectivity, graphLimit: graphLimit, onGraphLimitChange: onGraphLimitChange, renderedNodeCount: graphData?.nodes.length }), !graphData && (_jsx("div", { className: "absolute inset-0 flex items-center justify-center", children: _jsx("div", { className: "text-gray-500 text-sm", children: t('explorer:graph.loading') }) })), graphData && graphData.nodes.length === 0 && (_jsx("div", { className: "absolute inset-0 flex items-center justify-center", children: _jsx("div", { className: "text-gray-500 text-sm", children: t('explorer:graph.noData') }) }))] }));
}
/* ---- utility ---- */
