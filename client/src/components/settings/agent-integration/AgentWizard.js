...
// 配置向导
// ============================================================
export function AgentWizard({ onClose }) {
    ...
    const wizardCopiedTimerRef = useRef(null);
    // 手动流程用的状态
    ...
    const [codexVersion, setCodexVersion] = useState(null);
    const [geminiVersion, setGeminiVersion] = useState(null);
    ...
    const handleCreateAndNext = async () => {
        ...
        if (!agentName.trim() || !finalToolType.trim() || creating)
            return;
        setCreating(true);
        ...
        if (usePlugin) {
            ...
            try {
                ...
                if (result.success) {
                    ...
                    if (result.pluginsDir)
                        setPluginsDir(result.pluginsDir);
    ...
    useEffect(() => {
        return () => {
            if (wizardCopiedTimerRef.current)
                clearTimeout(wizardCopiedTimerRef.current);
        };
    }, []);
    ...
    return (_jsxs(Section, { title: t('agent.newAgent'), action: _jsx("button", { onClick: onClose, className: "text-gray-500 hover:text-gray-300 transition-colors", children: _jsx(X, { size: 16 }) }), children: [_jsx("div", { className: "flex items-center gap-1 mb-4", children: (effectiveToolType || step > 0) ? stepLabels.map((label, i) => (_jsxs("div", { className: "flex items-center", children: [_jsxs("div", { className: `flex items-center gap-1 px-2 py-1 rounded text-[10px] font-medium ${i === step ? 'bg-indigo-400/20 text-indigo-400' :
                                i < step ? 'text-emerald-400' : 'text-gray-500'}`, children: [i < step ? _jsx(Check, { size: 10 }) : _jsx("span", { children: i + 1 }), label] }), i < stepLabels.length - 1 && (_jsx(ChevronRight, { size: 12, className: "text-gray-600 mx-0.5" }))] }, i))) : (_jsx("span", { className: "text-[10px] text-gray-500", children: t('agent.wizard.selectToolHint') })) }), step === 0 && (_jsx(ToolSelectionStep, { agentName: agentName, setAgentName: setAgentName, toolType: toolType, setToolType: setToolType, customToolType: customToolType, setCustomToolType: setCustomToolType, effectiveToolType: effectiveToolType, usePlugin: usePlugin, creating: creating, onCreateAndNext: handleCreateAndNext })), step === 1 && usePlugin && createdAgent && (_jsx(AgentWizardPluginStep, { pluginGenerating: pluginGenerating, pluginError: pluginError, pluginGenerated: pluginGenerated, pluginDir: pluginDir, cliAvailable: cliAvailable, installing: installing, installResult: installResult, installCommand: installCommand, codexVersion: codexVersion, geminiVersion: geminiVersion, desktopConfigWritten: desktopConfigWritten, copied: copied, isCowork: isCowork, isCursor: isCursor, isCodex: isCodex, isWindsurf: isWindsurf, isOpenClaw: isOpenClaw, isGemini: isGemini, onInstallPlugin: handleInstallPlugin, onCopy: handleCopy, onPrevious: () => setStep(0), onNext: () => setStep(2) })), step === 1 && !usePlugin && createdAgent && (_jsx(ManualMcpConfigStep, { toolDef: toolDef, mcpSnippet: mcpSnippet, copied: copied, onCopy: handleCopy, onPrevious: () => setStep(0), onNext: () => setStep(2) })), step === 2 && !usePlugin && createdAgent && (_jsx(ManualSkillFileStep, { toolDef: toolDef, skillContent: skillContent, skillLoaded: skillLoaded, usingBaseFallback: usingBaseFallback, copied: copied, onCopy: handleCopy, onPrevious: () => setStep(1), onNext: () => setStep(3) })), step === verifyStep && createdAgent && (_jsx(AgentWizardVerifyStep, { createdAgent: createdAgent, usePlugin: usePlugin, pluginGenerated: pluginGenerated, desktopConfigWritten: desktopConfigWritten, pluginToolLabel: toolDef?.label ?? 'Claude Code', pluginVerifyMcp: toolDef?.pluginVerifyMcp, isManual: isManual, onPrevious: () => setStep(usePlugin ? 1 : 2), onClose: onClose }))] }));
}
