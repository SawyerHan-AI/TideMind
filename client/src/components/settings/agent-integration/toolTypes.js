...
export function isGeminiHooksReady(version) {
    if (!version)
        return false;
    const m = version.match(/^(\d+)\.(\d+)\.(\d+)/);
    if (!m)
