...
export function useAddNoteSourceConnection(step) {
    ...
    const [selectedAccountZpks, setSelectedAccountZpks] = useState(new Set());
    ...
    const selectToolType = useCallback((nextToolType) => {
    ...
    const checkAppleNotesPermission = useCallback(async () => {
        ...
        try {
            ...
            if (requestSeqRef.current !== seq)
                return;
            setPermissionResult({ accessible: false, path: '', error: err.message });
        }
        finally {
            if (requestSeqRef.current === seq)
                setCheckingPermission(false);
    ...
    const toggleAccount = useCallback((zpk) => {
    ...
    const setNotionToken = useCallback((token) => {
