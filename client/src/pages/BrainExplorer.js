...
export function BrainExplorer() {
    ...
    const [selectedId, setSelectedId] = useState(searchParams.get('node') || null);
    ...
    const { data: listData, loading } = useIPC(() => useSearchMode
    ...
    return (_jsxs("div", { className: "flex h-full gap-0", children: [_jsxs("div", { className: "flex-1 flex flex-col min-w-0 border-r border-white/5", children: [_jsx("div", { className: "px-4 pt-9 pb-3 border-b border-white/5", children: _jsx(FilterBar, { filter: filter, onFilterChange: handleFilterChange, viewMode: viewMode, onViewModeChange: setViewMode }) }), _jsx("div", { className: "flex-1 overflow-auto", children: viewMode === 'list' ? (_jsx(ListView, { nodes: listData?.nodes ?? [], total: listData?.total ?? 0, page: page, onPageChange: setPage, selectedId: selectedId, onSelect: setSelectedId, loading: loading })) : (_jsx("div", { className: "relative h-full", children: _jsx(GraphView, { filter: { ...filter, search: debouncedSearch || undefined, graphLimit }, selectedId: selectedId, onSelect: setSelectedId, graphLimit: graphLimit, onGraphLimitChange: setGraphLimit }) })) })] }), _jsx("div", { className: "w-[420px] flex-shrink-0 overflow-auto pt-8", children: _jsx(DetailPanel, { nodeId: selectedId, onNavigate: setSelectedId }) })] }));
}
